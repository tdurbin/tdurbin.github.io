window.ContextHubKernelConfig={debug:false,initializationTimeout:2000,stores:{geolocation:{type:"contexthub.geolocation",required:true,config:{geocoder:{enabled:true}}},surferinfo:{type:"contexthub.surferinfo",required:true},profile:{type:"granite.profile",required:true},emulators:{type:"granite.emulators",required:true},metadata:{type:"campaign.metadata",required:false},seeddata:{type:"campaign.seeddata",required:false},abandonedproducts:{type:"commerce.abandonedproducts",required:true},recentlyviewed:{type:"commerce.recentlyviewed",required:true},cart:{type:"commerce.cart",required:true},orderhistory:{type:"commerce.orderhistory",required:true},relatedproducts:{type:"commerce.relatedproducts",required:true},eventdata:{type:"aem.analyticsdata",required:true},pagedata:{type:"aem.pagedata",required:true},segmentation:{type:"aem.segmentation",required:true},cookie:{type:"contexthub.cookie",required:false,config:{cookies:[{name:"UserDetails",valueParserProvider:"URLParameterFormatUnsafe"},{name:"btRvp",valueParserProvider:"JSONFormat"}]}}}};
/*!
...
ContextHub.console.log(ContextHub.Shared.timestamp(),"[loading] contexthub.store.contexthub.cookie - store.cookie.js");
(function(e,c){var d;
var a={cookies:[{name:"Cookie1",valueParserProvider:"URLParameterFormat"},{name:"Cookie2",valueParserProvider:"URLParameterFormatUnsafe"},{name:"Cookie3",valueParserProvider:"JSONFormat"}]};
function b(g,f){this.config=e.extend(true,{},a,f);
this.init(g,this.config);
this.readData()
}ContextHub.Utils.inheritance.inherit(b,ContextHub.Store.PersistedStore);
b.prototype.readData=function(){d={};
for(var h=0;
h<this.config.cookies.length;
h++){var m=this.config.cookies[h].name;
var k=this.config.cookies[h].valueParserProvider;
d[m]={};
if(ContextHub.Utils.Cookie.exists(m)){var l=ContextHub.Utils.Cookie.getItem(m);
var f=[];
f=this["parseCookieValue_"+k](l);
for(var g=0;
g<f.length;
g++){d[m][f[g].key]=f[g].value
}}}this.addAllItems(d,{defer:0})
};
b.prototype.parseCookieValue_JSONFormat=function(k){var g=[];
var h=JSON.parse(k);
if(Array.isArray(h)){var f=[];
for(var j=0;
j<h.length;
j++){f.push({id:j+"",title:h[j],value:h[j]})
}g.push({key:"array",value:f})
}else{for(var j=0;
j<h.length;
j++){g.push({key:j,value:h[j]})
}}return g
};
b.prototype.parseCookieValue_URLParameterFormat=function(j){var f=[];
var h=j.split("&");
for(var g=0;
g<h.length;
g++){var k=h[g].split("=");
f.push({key:k[0],value:k[1]})
}return f
};
b.prototype.parseCookieValue_URLParameterFormatUnsafe=function(l){function n(r){var o=r.split("&");
var p=[];
function i(s){return(s.indexOf("=")>-1)&&!(s.indexOf("type=")>-1)
}function q(s){return s.replace("type=","")
}o.forEach(function(s){if(!i(s)){p.push(q(s))
}});
return p.join("&")
}var m=/[A-Z]{2}[&]{0,1}[A-Z]{0,2}[&]{0,1}[A-Z]{0,2}[&]?[A-Za-z]{0,}[&]?/;
var k=/[A-Z]{2}[-]{0,1}[A-Z]{0,2}[-]{0,1}[A-Z]{0,2}[-]?[A-Za-z]{0,}[-]?/;
var g=l.match(m);
if(g!=null){var j=g[0].replace(new RegExp("&","g"),"-");
l=l.replace(g,j)
}var f=this.parseCookieValue_URLParameterFormat(l);
for(var h=0;
h<f.length;
h++){if(f[h].value){var g=f[h].value.match(k);
if(g!=null){var j=g[0].replace(new RegExp("-","g"),"&");
f[h].value=f[h].value.replace(g,j)
}}}f.forEach(function(i){if(i.key=="type"){i.value=n(l)
}});
return f
};
b.prototype.reset=function(f){this.uber("reset",f);
...
ContextHub.Utils.storeCandidates.registerStoreCandidate(b,"contexthub.cookie",0)
}(ContextHubJQ,this));
